...
$Form1 = GUICreate("nhom 6", 615, 437, 192, 124)
	...
	Switch $nMsg
		...
		While 1
			   ...
			   $somay=GUICtrlRead($giatriA)
				...
				If $sodongIP0<999 Then _requetGooGleDOC('https://docs.google.com/document/d/1uJ60_uNbaaaG2Fs7gQC5v-EihcaVX4vBpWdaFjqPumE/export?format=txt','Keytinsoft.txt')
...
While 1   ; vong tra lai gia tr
	         ...
	         If $i6=1 Then
				$profileso=1
				;If $checkprofile=1 Then $profileso=5
				$i7=20
			EndIf
			If $i6=2 Then
				$profileso=21
				;If $checkprofile=1 Then $profileso=25
				$i7=40
			EndIf
			If $i6=3 Then
				$profileso=41
				;If $checkprofile=1 Then $profileso=45
				$i7=60
			EndIf
			If $i6=4 Then
				$profileso=61
				;If $checkprofile=1 Then $profileso=56
				$i7=80
			EndIf
			If $i6=5 Then
				$profileso=81
 ...
 For $i5=$profileso to $i7
				...
				If FileExists(@ScriptDir&'\checkUpdateCode.txt')=0 Then
				     _CheckUpdateCode('https://docs.google.com/document/d/1B_i5pSFoTGIAweoiqG7Hx1-AdWRkGmBbq7FJAe82AeA/export?format=txt')    ;thay doi theo nhom
				EndIf
				...
				Sleep(2000)
				_requetGooGleDOC('https://docs.google.com/document/d/1PTxYeX31wdMOaCiLBfPLuhNISjlLAHQpRleIRfAByiE/export?format=txt','linkkenhchaySUB2.txt')   ;thay doi theo nhom
	           ...
	           $thoigianDau=0
	  For $i=$Gmailso To 10
                 ToolTip('profile:'&$i5&'	 BLU so:'&$i&'	vongxoay:'&$vongxoaytho&'	nhom kenh:'&$nhomkenh&'	cummay 6'&'	vpsso:'&$vpsso,0,0)
				 $checkTG=1
                 $checkchonprofile=0
