...
$Form1 = GUICreate("nhom 10", 615, 437, 192, 124)
		...
		While 1
			   ...
			   $somay=GUICtrlRead($giatriA)
				...
				_CheckUpdateToolupdate('https://docs.google.com/document/d/1PStUUV13AuRmTXwykdKNy4iEaeW3gktWXn56zrUehfQ/export?format=txt')
				 ...
				 $checkrequet=_requetGooGleDOC('https://docs.google.com/document/d/1fkYnaUDL8Jx7cyuXWqPhok7a56NGKM0kVBL5rglKVx4/export?format=txt','Gmailtho.txt') ;thay doi theo nhom
                 _LayGmail($vpsso)
				...
				_requetGooGleDOC('https://docs.google.com/document/d/1vsP-AkEticxp6EQGlU_S-er5sa3oXXc_M83P4SPsERg/export?format=txt','Keytinsoft.txt')
				Sleep(2000)
				$sodongIP0=_FileCountLines(@ScriptDir&"\Keytinsoft.txt")
				If $sodongIP0<999 Then _requetGooGleDOC('https://docs.google.com/document/d/1vsP-AkEticxp6EQGlU_S-er5sa3oXXc_M83P4SPsERg/export?format=txt','Keytinsoft.txt')
...
While 1   ; vong tra lai gia tr
 ...
 For $i5=$profileso to $i7
				...
				If FileExists(@ScriptDir&'\checkUpdateCode.txt')=0 Then
				     _CheckUpdateCode('https://docs.google.com/document/d/1Wf7r310hs9wMkqOfSWB0jdVxhnVG8YuSAD5M3KCR9gA/export?format=txt')        ;thay doi theo nhom
				EndIf
	           ...
	           $thoigianDau=0
	  For $i=$Gmailso To 10
                 ToolTip('profile:'&$i5&'	 BLU so:'&$i&'	vongxoay:'&$vongxoaytho&'	nhom kenh:'&$nhomkenh&'	cummay 10'&'	vpsso:'&$vpsso,0,0)
				 $checkTG=1
                 $checkchonprofile=0
    ...
    Func _kiemtratrinhduyet($i5,$i,$vpsso,$trusomaybandau)
		...
		FileCopy($sSourceFile, 'C:\Users\'&@UserName&'\Desktop', 1)
			...
			If $check=0 And $check2=0 Then
				...
				For $i20=1 to 50
					$check=WinExists('Chào m?ng d?n v?i Firefox - Mozilla Firefox')
					If $check=1 Then
						Sleep(2000)
						$i20=50
						$check=1
...
#ce
			...
			If $check=0 Then
				...
				Sleep(15000)
				ControlClick('Ðang ch?y..., Trình cài d?t C?c C?c','Cài d?t','[CLASS:Button; INSTANCE:2]')
				Sleep(30000)
			...
			If $check=0 Then
				...
				Sleep(5000)
				WinClose('Khôi ph?c trang')
				Sleep(1000)

                WinSetState('Tab mo´i - Hô` so 1 - Microsoft? Edge','',@SW_RESTORE)
				Sleep(2000)
				WinMove('','',0,0,1366,768)
				Sleep(1000)
				_closeTrinhDuyet($i)
   ...
   EndFunc
		  ...
		  For $iSun2lan=1 to $sokenhsub1luot
			  ...
			  For $i10 = 1 to $var[0][0]
			    ...
			    If BitAnd (WinGetState ($var[$i10][1]), 2) And $var[$i10][0] <> "" Then
				    ...
				    WinClose('Khôi ph?c trang')
				    WinClose('B?n có mu?n khôi ph?c trang không?')
	    ...
	    Func _closeTrinhDuyet($i)
				...
				If $a=1 Then
					...
					Next
				EndIf
				$a = WinExists('Trình báo cáo l?i c?a Mozilla')                                                    ;x??o?b? tap
				If $a=1 Then
				    For $i20 = 1 to 10
					   WinClose('Trình báo cáo l?i c?a Mozilla')
					   Sleep(100)
					Next
				EndIf
				$a = WinExists('Ðóng Firefox')                                                    ;x??o?b? tap
				If $a=1 Then
				    For $i20 = 1 to 10
					   WinClose('Ðóng Firefox')
					   Sleep(100)
					Next
				EndIf
			...
			If $i=1 Then
				...
				Sleep(100)
				WinClose('Ðang ch?y..., Trình cài d?t C?c C?c')
                Sleep(3000)
                ControlClick('Cài d?t b? d?ng.','H?y Cài d?t','[CLASS:Button; INSTANCE:2]')
				Sleep(1000)
				ProcessClose('gAvastBrowser.exe')
                ProcessClose('gbrowser.exe')
				ProcessClose('gmsedge.exe')
